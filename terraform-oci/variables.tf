...
variable "node_source_image_ocid" {
  description = "OCI image ID, see https://docs.oracle.com/en-us/iaas/Content/ContEng/Reference/contengimagesshapes.htm#images__oke-images"
  type        = string
  nullable    = false
  default     = "ocid1.image.oc1.eu-frankfurt-1.aaaaaaaaisg5jmxrefsgaexzap4mm2spbenrjmeem6aoxescosf6wu7h4nuq"
  # Above is OKE ARM Oracle Linux 8.6 for K8s 1.24.1, via `oci ce node-pool-options get --node-pool-option-id all`
}

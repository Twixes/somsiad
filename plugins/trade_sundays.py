...
def determine_special_trade_sunday_dates(year: int) -> List[dt.date]:
    special_trade_sunday_dates = [
        ...
        determine_nearest_sunday_before_date(dt.date(year, 12, 25)),  # 1st Sunday before Christmas
    ]
    special_trade_sunday_dates.append(special_trade_sunday_dates[1] - ONE_WEEK)  # 2nd Sunday before Christmas
    if year == 2020:
...
def _determine_trade_sunday_dates_before_2018(
    ...
    if month is not None:
        current_sunday_date = determine_nearest_sunday_after_date(dt.date(year, month, 1))
        ...
        current_sunday_date = determine_nearest_sunday_after_date(dt.date(year, 1, 1))
...
def _determine_trade_sunday_dates_2018(
    ...
    if month is not None:
        if month < 3:
            ...
            first_sunday_of_month = determine_nearest_sunday_after_date(dt.date(year, month, 1))
        ...
        for i_month in range(3, 13):
            first_sunday_of_month = determine_nearest_sunday_after_date(dt.date(year, i_month, 1))
            if first_sunday_of_month not in exceptions:
                trade_sundays.append(first_sunday_of_month)
            last_sunday_of_month = determine_nearest_sunday_before_date(
...
def determine_trade_sunday_dates(year: int, month: int = None) -> List[dt.date]:
    ...
    if year < 2018:
        ...
        if year == 2018:
            ...
            trade_sundays = _determine_trade_sunday_dates_after_2019(exceptions, year, month)
        if month:

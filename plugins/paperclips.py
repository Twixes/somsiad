...
class Paperclips(commands.Cog, SomsiadMixin):
    def generate_paperclips_embed(self, clicks: int, machines: List[str]) -> discord.Embed:
        ...
        if machines:
            embed.add_field(name="Tempo produkcji spinaczy", value=f"{len(machines)*MACHINE_OUTPUT_PER_SECOND}/s", inline=False)
    ...
    async def paperclips(self, ctx: commands.Context):
        async def make_paperclip(interaction: Interaction):
            ...
            machines = [x.decode('utf-8') for x in (redis_connection.lrange(f"somsiad/paperclips/{interaction.guild_id}/machines", 0,-1) or [])]
            await interaction.edit_origin(embed=self.generate_paperclips_embed(clicks, machines), components=make_components(clicks, machines))
        ...
        async def buy_paperclip_machine(interaction: Interaction):
            ...
            if calculate_current_inventory(clicks, machines) >= MACHINE_COST:
                ...
                await interaction.respond(content=f"Nie masz wystarczająco spinaczy by zakupić maszynę! Jedna maszyna kosztuje {MACHINE_COST} spinaczy.")
        ...
        def make_components(clicks: int, machines: List[str]) -> List[Component]:
            ...
            components = [
                self.bot.components_manager.add_callback(
                    Button(emoji="📎", label="Zrób spinacz"), make_paperclip
                ),
            ]
            if machines or current_inventory>=MACHINE_COST:
                components.append(self.bot.components_manager.add_callback(
                    Button(emoji="🛠", label="Kup maszynę spinaczotworzącą (20 spinaczy)", disabled=current_inventory<MACHINE_COST), buy_paperclip_machine
                ))
            return components

        clicks = int(redis_connection.get(f"somsiad/paperclips/{ctx.guild.id}/clicks") or 0)
        machines = [x.decode('utf-8') for x in (redis_connection.lrange(f"somsiad/paperclips/{ctx.guild.id}/machines", 0,-1) or [])]
        message = await self.bot.send(ctx,
